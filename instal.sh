...
read -p "Masukkan Email Untuk Wordpress = " wpemail
echo "----------------------"
echo
...
apt install php$vphp-fpm php$vphp-common php$vphp-mysql php$vphp-xml php$vphp-xmlrpc php$vphp-curl php$vphp-gd php$vphp-imagick php$vphp-cli php$vphp-dev php$vphp-imap php$vphp-mbstring php$vphp-soap php$vphp-zip php$vphp-bcmath -y
echo
...
if [ $tipedomain == 1  ]
then
certbot --non-interactive -m ${wpemail} --agree-tos --no-eff-email --nginx -d ${domain} -d www.${domain} --redirect
else
certbot --non-interactive -m ${wpemail} --agree-tos --no-eff-email --nginx -d ${domain} --redirect
fi
echo
...
dbname="db_${domain//./}_$(pwgen 20 1)"
dbuser="user_${domain//./}_$(pwgen 20 1)"
dbpass=$(pwgen 20 1)
mysql << EOF
